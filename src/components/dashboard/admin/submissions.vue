...
import { openDialog } from '../../../modules/persists';
...
<div id="whole">
  ...
  <div id="submission-cont">
    <div class="submission" v-for="submission in LastAdminState.submissions.sort((a, b) => (b.score || 0) - (a.score || 0))" @mouseenter="(_e: MouseEvent) => { printingSFX.play() }">
      ...
      <p class="submission-meta submission-authors" v-if="!hideSubmissionSpoilers">Score: {{ submission.score }}</p>
      <p class="submission-meta submission-authors" v-if="(submission.modifiers || []).length > 0">Modifiers: {{ submission.modifiers?.map(mod => `${mod.modifier?.text} (${mod.modifier?.id})`).join(" - ") }}</p>
      ...
      <!-- <textarea disabled class="submission-meta-opaque submission-desc">{{ submission.desc }}</textarea> -->
       <a class="submission-meta-opaque submission-link" @click="e => {e.preventDefault(); openDialog(submission.title, submission.desc)}" href="#">Read Description</a>
